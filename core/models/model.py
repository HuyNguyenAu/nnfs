...
class Model:
    ...
    def train(self, x_train: np.ndarray, y_train: np.ndarray, *, epochs: int = 1, batch_size: int = 0, print_every: int = 100) -> None:
        '''
        Train the model.
        '''
        # The default value if batch size is not set.
    ...
    def prediction(self, x: np.ndarray, batch_size: int = 0) -> np.ndarray:
        '''
    ...
    def validate(self, x_val: np.ndarray, y_val: np.ndarray, batch_size: int = 0) -> None:
        '''
        Perform model validation.
        '''
        output: np.ndarray = self.prediction(x_val, batch_size)
