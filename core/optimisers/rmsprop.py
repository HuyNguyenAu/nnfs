...
class RMSProp(Optimiser):
    '''
    The root means squared propagation optimiser.
    '''

    def __init__(self, learning_rate: np.number = 1.0, decay: np.number = 0.0,
                 epsilon: np.number = 1e-7, rho: np.number = 0.9) -> None:
        '''
    ...
    def update_parameters(self, layer: Dense):
        ...
        # Update cache.
        layer.set_weights_cache(self.rho * layer.get_weights_cache() +
                                (1 - self.rho) * layer.get_d_weights() ** 2)
        layer.set_biases_cache(self.rho * layer.get_biases_cache() +
                               (1 - self.rho) * layer.get_d_biases() ** 2)
